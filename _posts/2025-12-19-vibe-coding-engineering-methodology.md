...
categories: [AI, 软件工程]
tags: [vibe-coding, ai-coding, intent-driven-development, 工程方法论, cursor, copilot]
description: "将 Vibe Coding 从'爽感操作'变成可复用、可验证、可回滚、可维护的工程流程，构建意图驱动开发(IDD)的完整方法论与实践模板。"
...
## 4. 维持上下文一致性：Memory Bank 是地基

Vibe Coding 最大的问题之一是"上下文漂移"：
你换个对话、过两天再继续、或者中途试过几个方向，模型很容易失忆或混淆。

解决方案就是 **Memory Bank（记忆库）**：把关键上下文固化成文件，让 AI 每次都"从同一份真相开始"。
...
### 4.2 三条硬规则（强烈建议写进 prd 或 architecture）

* **Always Read Rule**：每次开始任务前，AI 必须先读 `architecture.md`、`prd.md`、`progress.md`。
* **Progress is Source of Truth**：每完成一步就更新 `progress.md`（完成了什么、遇到什么坑、做了什么决策）。
...
`implementation-plan.md` 每一步必须包含：
...
正确姿势是：**每个大功能单独写一个 `feature-implementation.md`**，列出小步骤 + 验收。
...
1) 先阅读 memory-bank/prd.md、architecture.md、progress.md
2) 复述需求（含你不确定的点清单）
...
* 用规划避免架构漂移
* 用 Memory Bank 把上下文固定成可重复使用的"项目记忆"
* 用测试与验收把"感觉正确"变成"证据正确"
* 用 commit 把迭代变成"可回滚的进化"
* 用生产化加固把原型变成系统

下面是一套可以直接复制到项目里的 `memory-bank/` 初始文件模板：
...
### 📄 memory-bank/feature/feature-template.md
...
2. 把上面的模板复制进去，填入你的项目信息
3. 每次和 AI 对话前，让它先读这些文件
4. 每完成一步就更新 `progress.md` 并 commit
